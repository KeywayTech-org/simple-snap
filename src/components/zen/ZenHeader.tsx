...
export const ZenHeader: React.FC<ZenHeaderProps> = ({ currentStep, onReset }) => {
  ...
  return (
    <header className="w-full shrink-0 px-3 sm:px-8 md:px-12 py-2 sm:py-4 flex items-center justify-between border-b border-stone-200/70 bg-[#f7f5f0]/90 backdrop-blur-xs z-20 gap-2">
      {/* Brand logo in Chinese Calligraphic Serif */}
      <div className="flex items-center gap-2 sm:gap-3 shrink-0 min-w-0">
        <div className="flex items-center justify-center w-6 h-6 sm:w-8 sm:h-8 rounded border border-stone-800 text-stone-900 font-serif font-bold text-[11px] sm:text-sm bg-stone-100/50 shadow-2xs shrink-0 select-none">
          撕
        </div>
